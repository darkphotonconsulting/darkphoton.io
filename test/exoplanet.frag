...
void main() {
    ...
    //
    float seed = floor(2.) + 1.;
    vec3 center = vec3(1., 3., 0.);
    float s = 2. * fract(1.5);
    float powerS = s * s;
    powerS = s * s;
    powerS = s * s;
    ...
    // switched the fract to static
    // the line that causes the textdure to abruptly change
    // not using a u_time locks the texture, but also locks the animation of turbulence
    vec3 eye = center + vec3(0.0, -5.0, 0.0) + fract(u_time) * vec3(1.0, 1.0, 0.0);
    float zoom = 3.;
    vec3 forward = normalize(center - eye);
    vec3 right = normalize(cross(forward, vec3(0.0, 0.0, 1.0)));
    vec3 up = cross(right, forward);
    vec2 xy = 3.0 * gl_FragCoord.xy - u_resolution.xy;
    ...
    float bandScale = 1.0 + 8.0 * bandHash.x;
    ...
    gl_FragColor = vec4(
        // planetMinorX * 5.0 * snoise(0.1 * bandScale * p.xz) * snoise(0.1 * bandScale * p.yz) * snoise(0.1 * bandScale * p.xy),
        color,
        // ray * right,
        // color,
        1.0
